...
# Step 1: Fetch paper details from Scopus
def search_paper(paper_title):
    ...
    if search.results:
        ...
        return {
            ...
            'url': f'https://www.scopus.com/inward/record.uri?eid={paper.eid}',
        }
    return None

# New searching function
def search_papers_by_topic_and_year(topic, year):
    query = f'TITLE-ABS-KEY({topic}) AND PUBYEAR={year}'
    search = ScopusSearch(query)
    papers = []
    ...
    for paper in search.results:
        papers.append({
            'title': paper.title,
            'abstract': paper.abstract,
            'year': paper.coverDate.split('-')[0],
            'url': f'https://www.scopus.com/inward/record.uri?eid={paper.eid}',
        })
        if len(papers) >= 10:  # Limit to 10 papers per year
            break
    return papers
...
def compare_papers(paper_titles):
    ...
    # Identify the best paper for research based on key metrics
    best_paper = identify_best_paper(paper_results)
    print(f"Best paper for research: {best_paper['title']}")
